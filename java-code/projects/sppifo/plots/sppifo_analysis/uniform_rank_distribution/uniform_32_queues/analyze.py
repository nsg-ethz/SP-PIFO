...
def analyze_inversions():
    ...
    with open("temp/sppifo/sppifo_analysis/uniform_rank_distribution/uniform_32_queues/SPPIFO_32/inversions_tracking.csv.log") as file:
        reader = csv.reader(file)
        for row in reader:
            port = int(row[0])
            rank = int(row[1])
    ...
    with open("temp/sppifo/sppifo_analysis/uniform_rank_distribution/uniform_32_queues/Greedy_32/inversions_tracking.csv.log") as file:
        reader = csv.reader(file)
        for row in reader:
            port = int(row[0])
            rank = int(row[1])
    ...
    with open("temp/sppifo/sppifo_analysis/uniform_rank_distribution/uniform_32_queues/Fixed_queue_bounds_32/inversions_tracking.csv.log") as file:
        reader = csv.reader(file)
        for row in reader:
            port = int(row[0])
            rank = int(row[1])
